  ...
  <div class="flex flex-col lg:grid lg:grid-cols-2 gap-4 px-4 sm:px-0">
    ...
    <div class="text-gray-800 dark:text-gray-400 leading-relaxed text-justify bg-white dark:bg-gray-800 dark:bg-gradient-to-bl dark:from-gray-700/50 dark:via-transparent p-4 rounded-lg shadow-md flex flex-col gap-y-5">
      <div class="flex gap-x-10 flex-col md:flex-row">
        <div>
          {!! Status::from($work->work->status)->tailwindBadge() !!}
        </div>
        <a href="{{route('work.assigned-show', $work->work->id)}}" class="text-gray-950 dark:text-gray-100 font-bold text-lg hover:underline">{{$work->work->title}} </a>
        <p class="md:whitespace-nowrap">Publicado {{$work->work->created_at}}</p>
      </div>
      <div class="flex gap-x-2 items-center">
        <img class="h-8 w-8 rounded-full object-cover" src="{{ $work->work->client->profile_photo_url }}" alt="{{ $work->work->client->name}}" />
        <a href="{{route('user.show', ['user'=> $work->work->client->id])}}" class="dark:text-gray-200 hover:underline">{{$work->work->client->name}}</a>
      </div>
